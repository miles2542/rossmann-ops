...
with col_stat:
    ...
    if health_data:
        ...
        status_text = "API: Online | Model: Ready" if health_data.get("model_loaded") else "API: Online | Model: Missing"
    else:
        status_color = "red"
        status_text = "API: Offline"
    
    st.markdown(f"<div style='text-align: right; padding-top: 1.5rem;'><span style='color:{status_color};'>●</span> <b>{status_text}</b></div>", unsafe_allow_html=True)
...
with col_form:
    ...
    with st.form("forecast_form"):
        ...
        store_id = c1.number_input("Store ID", min_value=1, value=1, help="Unique store identifier")
        ...
        day_of_week = c3.selectbox("Day of Week", options=[1,2,3,4,5,6,7], index=default_dow-1, format_func=lambda x: ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"][x-1])

        c4, c5, c6 = st.columns(3)
        promo = c4.selectbox("Promo Active", options=[1, 0], format_func=lambda x: "Yes" if x == 1 else "No")
        holiday = c5.selectbox("State Holiday", options=["0", "a", "b", "c"], format_func=lambda x: {"0":"None", "a":"Public", "b":"Easter", "c":"Christmas"}[x])
        comp_dist = c6.number_input("Comp. Distance (m)", min_value=0.0, value=0.0, step=100.0, help="Distance to nearest competitor. Leave 0 to use stored default.")
...
with col_result:
    ...
    if not submitted:
        ...
        if not health_data:
            ...
            result_placeholder.error("Cannot predict: Model is missing on the backend. Needs retraining/export.")
        else:
            payload = {
                "Store": int(store_id),
            ...
            try:
                with st.spinner("Inference in progress..."):
                    res = requests.post(f"{st.session_state.api_url}/predict", json=payload)
                ...
                if res.status_code == 200:
                    ...
                    result_placeholder.error(f"API Error: {res.json().get('detail', res.text)}")
...
with st.expander("📊 Model Diagnostics & Explainability (SHAP)", expanded=False):
    st.markdown("Global feature importance plot. If predictions are heavily skewed by unexpected features, investigate data drift.")
    if st.button("Fetch SHAP Summary"):
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                st.warning("SHAP plot not found on server. Ensure model was exported correctly.")
        except:
