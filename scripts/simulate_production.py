...
def simulate_schema_error():
    ...
    try:
        ...
        if response.status_code == 422:
            logger.info("Success! API blocked malformed schema (422 Unprocessable Entity - Pydantic/Validation Error).")
            logger.info(f"Details: {response.json()}")
        else:
            logger.warning("Failed to trigger 422 validation. The API might have accepted it or failed differently.")
...
def simulate_attack():
    ...
    if max_z > 3.0:
        logger.error(f"Anomaly/Poisoning detected! Z-Score over threshold (>3). Aborting ingestion.")
        sys.exit(1)
    else:
...
def trigger_github_workflow():
    if not GITHUB_PAT:
        logger.warning("GITHUB_PAT environment variable not set. Cannot trigger GitHub Actions repository dispatch.")
        return
    ...
    if res.status_code == 204:
        ...
        logger.error(f"Failed to dispatch event. Code: {res.status_code}. Response: {res.text}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", choices=["schema", "attack", "drift"], required=True, 
