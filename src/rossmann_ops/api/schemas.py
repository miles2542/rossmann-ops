...
class PredictRequest(BaseModel):
    ...
    DayOfWeek: int = Field(
        ..., ge=1, le=7, description="Day of the week (1=Monday, 7=Sunday)."
    )
    ...
    Promo: int = Field(
        ...
        ge=0,
        le=1,
        description="Indicates whether a store is running a promo on that day.",
    )
    StateHoliday: str = Field(
        ...,
        description="Indicates a state holiday (a = public, b = Easter, c = Christmas, 0 = None).",
    )
    # Optional override for competition if known, otherwise uses store dataset defaults
    CompetitionDistance: Optional[float] = Field(
        None, ge=0, description="Distance in meters to the nearest competitor store."
    )
